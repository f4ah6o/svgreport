...
- The supplied repo-root `sample領収証.pdf` converts to SVG, but the imported text elements are path-only glyphs with empty `content`. That leaves `binding-candidate.md` without usable matches and makes automatic field-to-visual mapping effectively impossible without OCR or manual SVG annotation.
